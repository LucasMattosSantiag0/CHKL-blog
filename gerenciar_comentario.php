...
<html lang="pt-br">
  ...
  <div class="card">
    <div class="card-header">
      <ul class="nav nav-tabs card-header-tabs">
        <li class="nav-item ">
          <a class="nav-link text-dark active" href="gerenciar_comentario.php">Gerenciar Comentários</a>
        </li>
      </ul>
    </div>
    ...
    <section class="ml-5" >
      ...
      <div class="tabela-cod" style="max-width:68%">
        <table class="table table-bordered tabela-mat ">
          <thead> 
            <tr >
              <th class="table-dark col-md-10 h4">Comentários</th>
  ...
  $result = mysqli_query($db, "SELECT * FROM comentario order by id_comentario desc");
    while ($row = mysqli_fetch_array($result)) {
      echo "<tr class='table-light'>";
	  echo "<td>".$row['nome'].": ".$row['comentario']." </td>";
	  echo "<td>
<a href='./noticial.php?id=" .$row['id_post'] ."'> Matéria </a>
                <button class='btn-outline-dark border rounded ml-5'>
                  <a id='bot' href='./deletarc.php?id=" .$row['id_comentario'] ."' class='fas fa-trash-alt' ></a>
                </button>
              </td> ";
